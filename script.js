...
const instructors = [
  ...
  instrJsAvailOnWeekend = instructors.filter((instr) => {
    return (instr.availability === 'all' || instr.availability === 'weekend') && instr.specialities.includes('Javascript')
  })
  
  console.log(instrJsAvailOnWeekend)
  
  for (let i = 0; i < instrJsAvailOnWeekend.length; i++) {
    if (instrJsAvailOnWeekend[i].specialities.includes('Python')){
      console.log(`Hi ${instrJsAvailOnWeekend[i].name}, we inform you that this weekend you will be doing the support class and you need to prepare a Python workshop`)
    } else {
    console.log(`Hi ${instrJsAvailOnWeekend[i].name}, we inform you that this weekend you will be doing the support class`)
    }
  }